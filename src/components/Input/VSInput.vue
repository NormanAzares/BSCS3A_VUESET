<template>
    <div class="c-Input" :class="inputClassObject">
        <input ref="input" class="c-Input__input" type="text" :value="text" :name="name" :required="required"
            :disabled="disabled" :aria-label="label" :placeholder="placeholder" @input="text = $event.target.value" />
        <span class="c-Input__label" :title="label" @mousedown.prevent="onLabelClick($event)">
            {{ label }}
        </span>
    </div>
</template>
