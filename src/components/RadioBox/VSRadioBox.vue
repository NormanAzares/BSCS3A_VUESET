<template>
  <template v-for="(item, index) in items" :key="`radio-${index}`">
    <label class="c-Radio" v-bind="radioAttributes" @keydown.space.prevent @keyup.enter="handleChange(item.value)"
      @keyup.space="handleChange(item.value)">
      <input v-model="selectedValue" aria-hidden="true" class="c-Radio__input" type="radio" :name="name"
...
.c-Radio__element::after {
  ...
  height: 16px;
  border-radius: 16px;
  background-color: rgb(56, 107, 184);
  opacity: 0;
  transition: 0.15s;
}
